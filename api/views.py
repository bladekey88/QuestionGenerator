...
def generateAPIKey(request):
    ...
    if any("teacher" in s for s in user_groups):
        tokens = (
            AuthToken.objects.filter(user_id=request.user)
            .values_list("created", "expiry", "digest", "token_key")
            .order_by("-expiry")
        )
        ...
        messages.error(
            request,
            "<h4 class='text-center font-weight-bolder'>Access Denied</h4><h5 class='text-center'>Only teachers may use API functions.</h5>",
        )
...
@login_required
def expireAPIKey(request, digest):
    current_user_tokens = AuthToken.objects.filter(user_id=request.user)
    token = AuthToken.objects.get(pk=digest)
    if token in current_user_tokens:
        token.expiry = datetime.now()
        token.delete()
    return redirect("api:generate-api-key")
