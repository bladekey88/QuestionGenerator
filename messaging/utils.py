...
def fileUploadMessage(request, uploaded_url):
    ...
    <pre>Please do not reply to this this message as it has been automatically generated.</pre>""".format(
        request.user.first_name,
        uploaded_url[uploaded_url.rfind("/") + 1 : len(uploaded_url)],
    )
...
def fileProcessMessage(request, fileName):
    ...
    <pre>Please do not reply to this this message as it has been automatically generated.</pre>""".format(
        request.user.first_name, fileName[fileName.rfind("/") + 1 : len(fileName)]
    )
...
def fileProcessError(request, fileName):
    ...
    If you have any questions, please contact a staff member or raise a ticket from the <a target='_blank' href='https://127.0.0.1//help/index.php?a=add'>Help Menu</a>.
